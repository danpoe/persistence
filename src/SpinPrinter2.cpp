...
enum {
	DEFAULT_SPACE = Space(), BUFFER_SPACE, WA_SPACE,
	//OW_SPACE,
	SERVICE_SPACE
};
...
void SpinPrinter2::print(std::ostream &out, const Program &program) const {
	/* Set of used spaces. */
	...
	if (space == DEFAULT_SPACE || space == BUFFER_SPACE) {
		out << "byte mem" << space << "[" << program.memorySize() << "] = "
		<< Domain() << ';' << std::endl;
	} else
	out << "bit mem" << space << "[" << program.memorySize() << "] = "
	<< Domain() << ';' << std::endl;
